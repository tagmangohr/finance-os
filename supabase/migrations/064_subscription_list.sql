-- 064_subscription_list.sql
-- Paginated, segment-filtered customer list for the Subscriptions "Customers"
-- section (server pagination so every row is reachable page-by-page, filtered by
-- the DERIVED segment — active / past_due / churned / pending — not stored status),
-- plus an active contract-mix (monthly vs annual) function for the KPI strip.
-- Segment logic mirrors subscription_status_now (migrations 061-063), adding
-- 'pending' for never-activated mandates (status 'unknown'/null).

create or replace function subscription_list(
  p_org uuid,
  p_segment text default 'active',
  p_grace_months int default 6,
...
-- Active contract mix (monthly vs annual vs other) with MRR, for the KPI strip.
create or replace function subscription_mix_now(p_org uuid, p_grace_months int default 6)
returns table(billing_interval text, subs bigint, mrr numeric) language sql stable as $$
  with b as (
    select coalesce(lower(s.billing_interval), 'unknown') as billing_interval,
           _sub_mrr(s.amount_base, s.billing_interval) as mrr,
  ...
  select b.billing_interval, count(*)::bigint, coalesce(sum(b.mrr), 0)::numeric
  from b cross join t
  where not b.status_churned and b.p_end >= t.d      -- active only
  group by b.billing_interval;
$$;

grant execute on function subscription_list(uuid, text, int, text, text, int, int),
                         subscription_mix_now(uuid, int)
