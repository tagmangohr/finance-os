-- 065_subscription_list_stable_sort.sql
-- subscription_list offset pagination repeated rows: many subs share the same
-- amount_base (₹999) so the ORDER BY had ties → non-deterministic across pages.
-- Add subscription_id as a final stable tiebreaker.
...
$$;
grant execute on function subscription_list(uuid, text, int, text, text, int, int) to authenticated, anon, service_role;
