  ...
  return (
    <div className="space-y-3 max-w-[1400px]">
      ...
      <div className="animate-enter">
        ...
        <h1 className="text-[22px] font-bold tracking-tight text-foreground leading-none">Cash Flow</h1>
      </div>
      ...
      <Panel title="Cash Flow" subtitle="last 30 days">
        {cashFlowData.length > 0 ? (
            ...
            <div className="flex items-center gap-4 mt-1 px-1">
              {[
                ...
                <div key={label} className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10.5px] text-muted-foreground/70">{label}</span>
                </div>
              ))}
            </div>
          </>
          ...
          <div className="flex flex-col items-center justify-center h-52 gap-3">
            ...
            <p className="text-[13px] text-muted-foreground/70">No transaction data yet</p>
      ...
      {/* ── 3 forecast cards ─────────────────────────────────────────── */}
      <div>
        <p className="text-[10px] font-bold tracking-[0.16em] uppercase text-muted-foreground/70 mb-2 px-0.5">Projected Net Cash Flow</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 animate-enter-delay-2">
        ...
        <Panel title="Monthly Inflows vs Outflows" subtitle="P&amp;L summary">
          <div className="overflow-x-auto mt-1" style={{ gridColumn: "span 2" }}>
            {monthlyData.length === 0 ? (
              <div className="flex items-center justify-center h-24 text-[13px] text-muted-foreground/70">
              ...
              <table className="w-full text-[12px]">
                <thead>
                  <tr>
                    ...
                    <th className="text-right pb-2 text-[9.5px] font-bold tracking-[0.12em] uppercase text-muted-foreground/70">Inflow</th>
                    <th className="text-right pb-2 text-[9.5px] font-bold tracking-[0.12em] uppercase text-muted-foreground/70">Outflow</th>
                    <th className="text-right pb-2 text-[9.5px] font-bold tracking-[0.12em] uppercase text-muted-foreground/70">Net</th>
                  </tr>
                </thead>
                <tbody>
                  {monthlyData.map((row) => {
                    ...
                    return (
                      <tr key={row.month} className="border-t border-border group">
                        <td className="py-2 text-muted-foreground group-hover:text-muted-foreground transition-colors">{displayMonth}</td>
        ...
        <Panel title="Expense Categories">
          {categoryBreakdown.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-40 text-[13px] text-muted-foreground/70">
              No expense data yet
            </div>
          )}
        </Panel>
