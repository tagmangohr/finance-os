...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (!secretKey) {
    return NextResponse.json(
      { error: "Connector is missing secret_key in config" },
      { status: 422 }
    );
  }
  ...
  let charges: NormalizedTransaction[] = [];
  let payouts: NormalizedTransaction[] = [];
  const errors: string[] = [];

  const [chargesResult, payoutsResult] = await Promise.allSettled([
  ...
  if (chargesResult.status === "fulfilled") {
    ...
    errors.push(`charges: ${chargesResult.reason?.message ?? "unknown error"}`);
  }
