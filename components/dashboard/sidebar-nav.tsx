...
const NAV_GROUPS: NavGroup[] = [
  { label: "Overview", items: [
    ...
    { href: "/dashboard/intelligence", slug: "intelligence", label: "Intelligence", Icon: Sparkles },
  ]},
    ...
    <aside className="relative flex flex-col w-56 bg-card border-r border-border z-[1]">
      ...
      <div className="flex-1 overflow-y-auto py-2 space-y-1">
        {groups.map((g) => (
          <div key={g.label}>
            <div className="px-3 pt-2 pb-1">
              <span className="text-[9.5px] font-bold tracking-[0.16em] text-muted-foreground/70 uppercase">{g.label}</span>
            </div>
        ...
        {setup.length > 0 && (
          <div>
            <div className="px-3 pt-2 pb-1">
              <span className="text-[9.5px] font-bold tracking-[0.16em] text-muted-foreground/70 uppercase">Setup</span>
            </div>
            <nav className="px-2.5 space-y-px">
      ...
      {connectorCount > 0 && (
        <div className="mx-2.5 mb-2 p-2.5 border border-border rounded-lg bg-accent/40">
          <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
            <span className="w-1.5 h-1.5 rounded-full bg-success flex-shrink-0" />
            <span className="flex-1 font-medium text-foreground/80">{connectorCount} connector{connectorCount !== 1 ? "s" : ""}</span>
      ...
      <div className="p-2.5 border-t border-border">
        <Link
          ...
          className="flex items-center gap-2 px-2 py-1.5 mb-1 rounded-lg hover:bg-accent transition-colors group"
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[12px] font-medium text-foreground/80 truncate group-hover:text-foreground transition-colors">{displayName}</p>
            <p className="text-[10px] text-muted-foreground truncate">{userEmail}</p>
          </div>
        </Link>
        <button
          onClick={handleSignOut}
          className="flex items-center gap-2 w-full px-2 py-1.5 rounded-lg text-[12px] text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-colors duration-150"
        >
